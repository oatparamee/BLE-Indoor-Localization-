 ...
 *
 * The app is no longer limited to the three hard-coded Beacon_A/B/C names —
 * any beacon the scanner sees can be selected by UUID and assigned a
 * position (x, y) to be used as the 1st / 2nd / 3rd beacon during
 * trilateration.
 */
...
class BLEScanner {
  ...
  async startScanning(cb: ScanCallback): Promise<void> {
    // The scanner is a singleton shared across all screens. If a scan
    // is already in progress, just swap the callback so the newest
    // listener receives updates — do NOT start a second underlying
    // BLE scan (that would leak intervals + double permission prompts).
    this.callback = cb;
    if (this.scanning) {
      if (this.callback) {
    ...
    this.updateInterval = setInterval(() => {
      ...
      if (this.callback) {
        ...
        this.callback(copy, this._getNearbyList());
      }
    }, 1000);
  }
