...
export default function CalibrationScreen() {
  ...
  const startCollecting = useCallback(async () => {
    ...
    collectIntervalRef.current = setInterval(async () => {
      ...
      const beacon = readings[selectedBeacon];

      if (!beacon || beacon.rawRssi === null) {
        setStatusMessage(`Waiting for ${selectedBeacon}...`);
        return;
      }
  ...
  }, [knownDistance, maxSamples, selectedBeacon]);
        ...
        <View style={styles.beaconSelector}>
          {detectedBeacons.map(name => (
            <TouchableOpacity
              ...
              style={[
                styles.beaconButton,
                selectedBeacon === name && styles.beaconButtonActive,
              ]}
              onPress={() => setSelectedBeacon(name)}>
              <Text
                style={[
                  styles.beaconButtonText,
                  selectedBeacon === name && styles.beaconButtonTextActive,
                ]}>
                {name}
              </Text>
            </TouchableOpacity>
